<template>
  <f7-page>
    <f7-navbar>
      ...
      <f7-nav-right>
         <f7-button @click="logout()">Đăng xuất</f7-button>
      </f7-nav-right>
    </f7-navbar>
...
export default {
  ...
  computed:{
    ...mapGetters(["getView", "getFeatureLayer", "get_page_image"])
  },
  watch:
  {
    get_page_image(newVal) {
      console.log(newVal)
    },
    ...
    getView(newVal)
    {
      ...
      newVal.when( () => {
        console.log ('123')
          newVal.popup.on("trigger-action",  (event) => {
              console.log('test',event)
              if(event.action.id == "updateInfor")
              {
                  app.$store.state.BaseMap.dialog_update = true
                  app.$store.state.BaseMap.selectedFeature = event.target.selectedFeature
                  console.log('test',app.$store.state.BaseMap.selectedFeature)
                  app.$f7router.navigate("/sua-cay-xanh/"+event.target.selectedFeature.attributes.OBJECTID+"/")
              }
              else if(event.action.id == 'viewImage')
              {
